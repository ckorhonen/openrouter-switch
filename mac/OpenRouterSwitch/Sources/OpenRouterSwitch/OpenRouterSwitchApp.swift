...
@main
struct OpenRouterSwitchApp: App {
...
#endif
        OpenRouterSwitchApp.exitIfAlreadyRunning(variant: .current())
    }
    ...
    static func exitIfAlreadyRunning(variant: AppVariant) {
        ...
        FileHandle.standardError.write(Data(
            "[OpenRouterSwitch] another instance is already running (pid \(other.processIdentifier)); exiting\n".utf8))
        exit(0)
    }
    ...
    static let menubarIcon: NSImage = {
        guard let url = menubarIconResourceURL(),
            ...
            return NSImage(systemSymbolName: "circle.lefthalf.filled",
                           accessibilityDescription: "OpenRouter Switch") ?? NSImage()
        }
        img.isTemplate = true
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private var state: OpenRouterSwitchState?
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let state = OpenRouterSwitchState(variant: variant)
        let routerWindowController = RouterWindowController(
            ...
            windowOpenChanged: { isOpen in
                ...
                if !NSApp.setActivationPolicy(policy) {
                    FileHandle.standardError.write(Data(
                        "[OpenRouterSwitch] failed to set activation policy to \(policy.rawValue)\n".utf8))
                }
    ...
    private func showPreview(_ fixture: PopupPreviewFixture) {
        NSApp.setActivationPolicy(.accessory)
        if ProcessInfo.processInfo.environment["OPENROUTER_SWITCH_POPUP_APPEARANCE"] == "dark" {
            NSApp.appearance = NSAppearance(named: .darkAqua)
        }
        let variant = AppVariant.current()
        let state = OpenRouterSwitchState(preview: fixture, variant: variant)
        let controller = StatusItemController(
            state: state, variant: variant, isPreview: true)
        self.state = state
        previewController = controller
        let hasHostWindow =
            ProcessInfo.processInfo.environment["OPENROUTER_SWITCH_POPUP_HOST_WINDOW"] == "1"
        if hasHostWindow {
            let host = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            host.title = "OpenRouter Switch Menu Fixture"
