...
@MainActor
final class OpenRouterSwitchState: ObservableObject {
    ...
    init(variant: AppVariant = .current(),
        ...
        if variant.channel == .preview,
           ...
           Self.locateOpenRouterSwitchBinary(variant: variant) == nil {
            lastError = "OpenRouter Switch Preview requires OPENROUTER_SWITCH_GATEWAY_BIN from its launcher."
        }
    ...
    func startSystem() async {
        ...
        guard result.succeeded else {
            lastError = result.timedOut
                ? "Starting OpenRouter Switch timed out."
                : "OpenRouter Switch could not be started."
            return
        }
    ...
    private func executeCLI(
        ...
        guard let binary = Self.locateOpenRouterSwitchBinary(variant: variant) else {
            lastError = variant.channel == .preview
                ? "OpenRouter Switch Preview requires OPENROUTER_SWITCH_GATEWAY_BIN from its launcher."
                : "openrouter-switch is not installed."
            return failedCLIResult
        }
    ...
    private var runtimeTrustError: String {
        ...
        case .stable, .previewTrusted:
            return ""
        case .previewDown:
            return "OpenRouter Switch Preview is not running."
    ...
    nonisolated static func locateOpenRouterSwitchBinary(
        variant: AppVariant = .current()
    ) -> URL? {
        if variant.channel == .preview {
            guard let path = variant.runtime.environment["OPENROUTER_SWITCH_GATEWAY_BIN"],
        ...
        if let raw = variant.runtime.environment["OPENROUTER_SWITCH_GATEWAY_BIN"],
           !raw.isEmpty {
            candidates.append(raw)
        } else if let raw = ProcessInfo.processInfo.environment["OPENROUTER_SWITCH_GATEWAY_BIN"],
           !raw.isEmpty {
            candidates.append(raw)
        }
        candidates.append("\(NSHomeDirectory())/.local/bin/openrouter-switch")
        candidates.append("/opt/homebrew/opt/openrouter-switch/bin/openrouter-switch")
        candidates.append("/usr/local/opt/openrouter-switch/bin/openrouter-switch")
        return candidates
...
func cliEnvironmentOverrides(configPath: String) -> [String: String] {
    configPath.isEmpty ? [:] : ["OPENROUTER_SWITCH_CONFIG_PATH": configPath]
}
...
func runtimeTrustForSnapshot(
    ...
    let expectedEnvironment = [
        "OPENROUTER_SWITCH_CONFIG_PATH": variant.runtime.expectedConfigPath ?? "",
