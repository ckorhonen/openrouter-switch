...
final class DisplayTests: XCTestCase {
    ...
    func testRegularApplicationHasConventionalMainMenu() {
        let hierarchy = makeApplicationMainMenu(
            displayName: "OpenRouter Switch Preview")

        XCTAssertEqual(
            hierarchy.menu.items.map(\.title),
            ["OpenRouter Switch Preview", "File", "Edit", "Window"])
        XCTAssertEqual(
            hierarchy.menu.items.first?.submenu?.items.first?.title,
            "About OpenRouter Switch Preview")
        XCTAssertTrue(
            hierarchy.menu.items.first?.submenu?.items.contains {
                $0.title == "Quit OpenRouter Switch Preview"
                    && $0.keyEquivalent == "q"
            } ?? false)
        XCTAssertTrue(
    ...
    func testGlobalRoutingTogglePresentationForMenuSurface() {
        let state = OpenRouterSwitchState(preview: .healthy)
    ...
    func testCLIEnvironmentTargetsRunningRouterConfig() {
        ...
        XCTAssertEqual(
            ...
            ["OPENROUTER_SWITCH_CONFIG_PATH": "/tmp/live/gateway.yaml"])
    }
    ...
    func testMenuErrorLabel() {
        ...
        XCTAssertEqual(menuErrorLabel("openrouter-switch on codex failed (exit 1)"),
                       "openrouter-switch on codex failed (exit 1)")
    ...
    func testPreviewVariantUsesIndependentIdentityAndRuntime() {
        let variant = AppVariant.resolve(
            infoDictionary: [
                "OpenRouterSwitchBuildChannel": "preview",
        ...
        XCTAssertEqual(
            ...
            "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/gateway.yaml")

        let environment = variant.runtime.environment
        XCTAssertEqual(environment["OPENROUTER_SWITCH_CONFIG_PATH"],
                       variant.runtime.expectedConfigPath)
        XCTAssertEqual(environment["OPENROUTER_SWITCH_GATEWAY_BIN"],
                       "/workspace/bin/openrouter-switch")
        XCTAssertEqual(environment["OPENROUTER_SWITCH_GATEWAY_ADMIN"],
                       "http://127.0.0.1:45373")
        XCTAssertEqual(environment["OPENROUTER_SWITCH_GATEWAY_PORT"], "45373")
        XCTAssertEqual(environment["OPENROUTER_SWITCH_DOOR_PORTS"], "45371")
        XCTAssertEqual(environment["OPENROUTER_SWITCH_GATEWAY_PID"],
                       "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/gateway.pid")
        XCTAssertEqual(environment["OPENROUTER_SWITCH_TELEMETRY_DIR"],
                       "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/telemetry")
    ...
    func testPreviewDoesNotInheritAmbientStableEndpoints() {
        let variant = AppVariant.resolve(
            infoDictionary: ["OpenRouterSwitchBuildChannel": "preview"],
            homeDirectory: "/tmp/openrouter-switch-home",
            environment: [
                "OPENROUTER_SWITCH_ADMIN_URL": "http://127.0.0.1:45273",
            ])

        XCTAssertEqual(adminBaseURL(runtime: variant.runtime),
                       URL(string: "http://127.0.0.1:45373/")!)
        XCTAssertNil(variant.runtime.environment["OPENROUTER_SWITCH_GATEWAY_BIN"])
        XCTAssertNil(OpenRouterSwitchState.locateOpenRouterSwitchBinary(variant: variant))
    }
    ...
    func testPreviewCLIEnvironmentIsCompleteAndIgnoresReportedConfig() {
        ...
        XCTAssertEqual(overrides["OPENROUTER_SWITCH_CONFIG_PATH"],
                       "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/gateway.yaml")
        XCTAssertEqual(overrides["OPENROUTER_SWITCH_GATEWAY_PIDFILE"],
                       "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/gateway.pid")
        XCTAssertEqual(overrides["OPENROUTER_SWITCH_DOOR_PIDFILE"],
                       "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/door.pid")
        XCTAssertNil(overrides["OPENROUTER_SWITCH_TIER_FILE"])
        XCTAssertNil(overrides["OPENROUTER_SWITCH_TIERS_FILE"])
    }

    func testPreviewRuntimeTrustFailsClosed() {
        let expected = "/tmp/openrouter-switch-home/.config/openrouter-switch-preview/gateway.yaml"
        XCTAssertEqual(
    ...
    func testPreviewMenubarIconKeepsStableOpticalSizeWithBadge() {
        let preview = AppVariant.resolve(
            ...
            homeDirectory: "/tmp/openrouter-switch-home",
            environment: [:])
        let image = OpenRouterSwitchApp.menubarIcon(for: preview)
    ...
    // The candidate order ($OPENROUTER_SWITCH_GATEWAY_BIN, ~/.local/bin, brew opt
    ...
    func testLocateBinaryReturnsExecutableOrNil() {
        if let url = OpenRouterSwitchState.locateOpenRouterSwitchBinary() {
    ...
    func testSubagentRoutingDescriptionExplainsNoRewriteAndSavedOverride() {
        ...
        XCTAssertEqual(
            subagentRoutingDescription(
                client: noOverride,
                globalRoutingEnabled: true),
            "No subagent override. Claude Code chooses the model, then OpenRouter Switch applies that model’s family mapping or the default mapping.")
        XCTAssertEqual(
            subagentRoutingDescription(
                client: noOverride,
