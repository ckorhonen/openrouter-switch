...
// `openrouter-switch menubar` verb plus the whole-system up/down steps that
...
// `openrouter-switch menubar` or `openrouter-switch up`.
//
// Resolution order for the source bundle: $OPENROUTER_SWITCH_MENUBAR_APP (opened in
...
// up/down quit any OpenRouterSwitch instance the current user runs.
// OPENROUTER_SWITCH_MENUBAR_APP (no keg copy touched) and
// OPENROUTER_SWITCH_MENUBAR=off isolate development sessions.
package main
...
const menubarAppName = "OpenRouter Switch.app"
...
const menubarProcName = "OpenRouterSwitch"
...
// relaunches the user's real menubar app, mirroring OPENROUTER_SWITCH_LAUNCHD=off.
func menubarDisabled() bool { return os.Getenv("OPENROUTER_SWITCH_MENUBAR") == "off" }
...
var errMenubarAppMissing = errors.New("app not found. Install it with '" + menubarInstallHint + "' (or set OPENROUTER_SWITCH_MENUBAR_APP to a built bundle)")
...
const menubarUpgradeHint = "brew upgrade openrouter-switch"
...
// resolved to:
//
//	.../Cellar/openrouter-switch/<version>/bin/openrouter-switch
//
...
func homebrewMenubarPayload() (payload, prefix string, homebrew bool) {
	...
	if filepath.Base(executable) != "openrouter-switch" || filepath.Base(filepath.Dir(executable)) != "bin" {
		return "", "", false
	}
	prefix = filepath.Dir(filepath.Dir(executable))
	formulaDir := filepath.Dir(prefix)
	if filepath.Base(formulaDir) != "openrouter-switch" || filepath.Base(filepath.Dir(formulaDir)) != "Cellar" {
		return "", "", false
	}
	return filepath.Join(prefix, "share", "openrouter-switch", menubarPayloadName), prefix, true
}
...
type menubarTarget struct {
	...
	managed    bool   // path is the ~/Applications copy (not the OPENROUTER_SWITCH_MENUBAR_APP override)
...
func resolveMenubarTarget() (menubarTarget, error) {
	if v := os.Getenv("OPENROUTER_SWITCH_MENUBAR_APP"); v != "" {
		if _, err := os.Stat(v); err != nil {
			return menubarTarget{}, fmt.Errorf("OPENROUTER_SWITCH_MENUBAR_APP=%s does not exist", v)
		}
...
func materializeMenubarPayload(payload, appDir string) (app string, cleanup func(), err error) {
	...
	stage, err := os.MkdirTemp(appDir, ".openrouter-switch-menubar.")
	if err != nil {
		return "", func() {}, err
	}
...
func cmdMenubar(args []string) int {
	// --which prints the openrouter-switch binary the menubar app will resolve
	...
	if len(args) == 1 && args[0] == "--which" {
		...
		if p == "" {
			fmt.Fprintln(os.Stderr, "menubar: no openrouter-switch binary found in the menubar lookup order")
			return 1
		}
		fmt.Println(p)
		return 0
	...
	if len(args) > 0 {
		fmt.Fprintln(os.Stderr, "usage: openrouter-switch menubar [--which]")
		return 2
	}
...
// under OPENROUTER_SWITCH_MENUBAR=off). Not running: open the freshly refreshed
...
// "Start OpenRouter Switch" action (it runs `up`) safe against self-kill
// loops.
func upMenubar() int {
...
// (a `openrouter-switch menubar` whose open only activated the survivor, an
...
// menubarPid returns the pid of a running OpenRouterSwitch process owned by
...
// to open: the OPENROUTER_SWITCH_MENUBAR_APP override, a Homebrew keg, or the
...
// executable (".../OpenRouter Switch.app/Contents/MacOS/OpenRouterSwitch"), and the
...
// the row does not apply (off-darwin, or OPENROUTER_SWITCH_MENUBAR=off), otherwise
...
func menubarStatusLines() []string {
	...
	pid := menubarPid()
	if pid == 0 {
		// "fix: openrouter-switch up" is a dead end when there is no bundle to
		...
		if !menubarInstalled() {
			...
			// under OPENROUTER_SWITCH_MENUBAR_APP, which bypasses the keg entirely
			// (menubarInstalled already answered for the override).
			if os.Getenv("OPENROUTER_SWITCH_MENUBAR_APP") == "" {
				if kegDir := menubarKegDirWithoutApp(); kegDir != "" {
		...
		return []string{"DOWN (fix: openrouter-switch up)"}
	}
	...
	if menubarInstanceStale(pid, bundle) {
		return []string{
			fmt.Sprintf("up (pid %d) STALE: instance predates the installed copy (%s on disk; fix: openrouter-switch up)", pid, label),
			bundle,
		}
	}
