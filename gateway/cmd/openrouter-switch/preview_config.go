...
const previewConfigUsage = "usage: openrouter-switch config preview-snapshot --source PATH --output PATH --root PATH --router-addr HOST:PORT --door-addr HOST:PORT | openrouter-switch config preview-validate --path PATH --root PATH --router-addr HOST:PORT --door-addr HOST:PORT"
