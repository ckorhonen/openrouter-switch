...
// ~/.config/openrouter-switch are never read or written. The subagents verb is
// config-edit (gateway.yaml), not settings-write, so these tests build a
// temp gateway.yaml via OPENROUTER_SWITCH_CONFIG_PATH and exercise the verb through
...
`
	...
	t.Setenv("OPENROUTER_SWITCH_BACKUP_DIR", backupDir)
