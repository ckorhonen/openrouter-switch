...
// temp gateway.yaml via OPENROUTER_SWITCH_CONFIG_PATH, temp settings.json via
// OPENROUTER_SWITCH_CLAUDE_SETTINGS, a stub signalRouter recorder, and a fake admin
// server serving model_routes for the verify poll (or a router-down
// path). The real ~/.claude and ~/.config/openrouter-switch are never touched.
...
func newRouteTestEnv(t *testing.T, adminDown bool) *routeTestEnv {
	...
	if !adminDown {
		...
		t.Setenv("OPENROUTER_SWITCH_ADMIN_ADDR", hostPort(t, adminSrv.URL))
	} else {
...
`
	...
	t.Setenv("OPENROUTER_SWITCH_CLAUDE_SETTINGS", settings)
