...
// state): OPENROUTER_SWITCH_CODEX_HOME (overlay dir, default ~/.codex),
// OPENROUTER_SWITCH_BACKUP_DIR (backup root; the codex/ subdir is appended),
// OPENROUTER_SWITCH_ENV_FILE (the gateway's local placeholder-token source), and
// OPENROUTER_SWITCH_CONFIG_PATH (gateway.yaml, as everywhere else).
package main
...
func codexDoorPort(f *config.File) (*config.Client, string, map[string]bool, error) {
	...
	if target == nil {
		return nil, "", nil, fmt.Errorf("no openai-shape client in gateway.yaml; paste this codex client into the clients: list (bind_addr must match your router listener; this is the shipped template's parked block) and re-run 'openrouter-switch codex on', which will offer to enable it:\n\n%s\n", codexClientYAML)
	}
	if f.Door == nil || len(f.Door.Ports) == 0 {
		return nil, "", nil, fmt.Errorf("gateway.yaml has no door: section; the codex adapter points the harness at the front door, so add door.ports mapping a bind_addr to the %s router listener %s (reference: config/gateway.example.yaml in the openrouter-switch repo)", target.Name, target.BindAddr)
	}
	for _, dp := range f.Door.Ports {
...
func codexDefaultSlug(c *config.Client) (string, error) {
	...
	return "", fmt.Errorf("no default_model configured for the %s client in gateway.yaml (reference: config/gateway.example.yaml in the openrouter-switch repo)", c.Name)
}
...
func (a *codexAdapter) on() int {
	...
	if ours && !managed {
		if nb != nil {
			fmt.Fprintf(a.out, "note: pre-existing %s is openrouter-switch-shaped but stale (base_url not on a live gateway port); backed it up and replacing it\n", a.overlayPath)
		} else {
			...
			fmt.Fprintf(a.out, "note: %s is openrouter-switch-shaped but stale (base_url not on a live gateway port); replacing it WITHOUT a new backup (the first-'on' backup already holds your pre-'on' original). Its content was:\n%s\n",
...
func (a *codexAdapter) unpark() int {
	...
	if !strings.EqualFold(strings.TrimSpace(resp), "y") {
		fmt.Fprintf(a.out, "left parked; re-run 'openrouter-switch codex on' to enable it later, or set enabled: true on the %s client in %s and SIGHUP the router\n", a.clientName, a.configPath)
		return 0
	}
...
func (a *codexAdapter) enableReloadAndVerify() {
	...
	case pidfileAlive:
		...
		if waitClientBound(adminAddr, a.clientName, routeApplyTimeout) {
			...
			fmt.Fprintf(a.out, "note: SIGHUP sent (pid %d) but the router did not report a bound %s listener within %s; the reload may have failed (check the router log and 'openrouter-switch status')\n", pid, a.clientName, routeApplyTimeout)
		}
	default:
...
func (a *codexAdapter) status(stdout io.Writer) int {
	...
	overlayLabel := "absent (default state; 'openrouter-switch codex on' creates it)"
	switch {
	case managed:
		overlayLabel = "managed (openrouter-switch overlay)"
	case ours:
		overlayLabel = fmt.Sprintf("openrouter-switch-shaped but stale (base_url %s is not a live gateway port)", sh.baseURL)
	case existed:
		overlayLabel = "present, not openrouter-switch-managed"
	}
	clientLabel := "enabled"
	if !a.clientEnabled {
