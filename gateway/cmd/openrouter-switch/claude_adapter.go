// claude_adapter.go implements `openrouter-switch claude on|off|status` and
// `openrouter-switch claude subagents [<model>|off]`.
//
...
// state): OPENROUTER_SWITCH_CLAUDE_SETTINGS (settings.json path), OPENROUTER_SWITCH_BACKUP_DIR
// (backup root; the claude/ subdir is appended), OPENROUTER_SWITCH_CONFIG_PATH
...
func claudeTargetClientName(f *config.File) (string, error) {
	...
	if target == nil {
		return "", fmt.Errorf("no anthropic-shape client in gateway.yaml; add a claude-code client (reference: config/gateway.example.yaml in the openrouter-switch repo)")
	}
	return target.Name, nil
}

// loadGatewayConfigForAdapter resolves gateway.yaml the same way the
// lifecycle commands do: OPENROUTER_SWITCH_CONFIG_PATH, else the sticky recorded
// path, else the default.
func loadGatewayConfigForAdapter() (*config.File, string, error) {
	path := os.Getenv("OPENROUTER_SWITCH_CONFIG_PATH")
	if path == "" {
...
func claudeDoorPort(f *config.File) (string, map[string]bool, map[string]string, error) {
	...
	if target == nil {
		return "", nil, nil, fmt.Errorf("no anthropic-shape client in gateway.yaml; add a claude-code client (reference: config/gateway.example.yaml in the openrouter-switch repo)")
	}
	if f.Door == nil || len(f.Door.Ports) == 0 {
		return "", nil, nil, fmt.Errorf("gateway.yaml has no door: section; the claude adapter points the harness at the front door, so add door.ports mapping a bind_addr to the %s router listener %s (reference: config/gateway.example.yaml in the openrouter-switch repo)", target.Name, target.BindAddr)
	}
	for _, dp := range f.Door.Ports {
...
func (a *claudeAdapter) on() int {
	...
	if changed {
		...
		fmt.Fprintln(a.out, "restart Claude Code sessions to pick this up; revert with 'openrouter-switch claude off'.")
	} else {
...
func (a *claudeAdapter) subagentsSetModel(model string, opts mutationOptions, stdout io.Writer, journaled bool) int {
	...
	if class == subagentClassAlias {
		if _, configured := a.modelAliases[model]; !configured {
			fix := "add it to model_aliases for the " + a.clientName + " client in gateway.yaml and SIGHUP the router (kill -HUP $(cat ~/.config/openrouter-switch/gateway.pid)), or pick a configured alias"
			message := ""
...
// state is displayed as inherit: OpenRouter Switch performs no subagent-specific
...
func (a *claudeAdapter) subagentsRouting(want, requestedTarget string, opts mutationOptions, stdout io.Writer, journaled bool) int {
	...
	if want == "off" && cur.SubagentModel == "" {
		...
		fmt.Fprintf(a.out, "claude subagents: already inherit (no subagent_model configured; no subagent-specific model rewrite; requested models follow family mappings or the unmatched-model default; set one with 'openrouter-switch claude subagents <model>')\n")
		return 0
	}
...
func (a *claudeAdapter) subagentsReloadAndVerify(wantModel, wantRouting string) {
	...
	case pidfileAlive:
		...
		if waitSubagentApplied(adminAddr, a.clientName, wantModel, wantRouting, routeApplyTimeout) {
			...
			fmt.Fprintf(a.out, "note: SIGHUP sent (pid %d) but the router did not confirm the subagent config within %s; check 'openrouter-switch status'\n", pid, routeApplyTimeout)
		}
	default:
...
func (a *claudeAdapter) subagentsWarnings() {
	...
	if err == nil && existed {
		if env, envErr := settingsEnv(root); envErr == nil {
			if v, ok := envString(env, claudeSubagentEnvKey); ok && v != "" {
				fmt.Fprintf(a.out, "warning: %s is set in %s; the gateway rewrite overrides it while toggled on, and the pin still routes explicitly while toggled off. Suggest removing it ('openrouter-switch claude off' strips a gateway-owned value).\n", claudeSubagentEnvKey, a.settingsPath)
			}
			if base, ok := envString(env, claudeManagedEnvKey); ok && !a.isGatewayURL(base) {
				fmt.Fprintf(a.out, "warning: %s does not point at the gateway; the subagent toggle has no effect until 'openrouter-switch claude on'.\n", claudeManagedEnvKey)
				wiringWarned = true
			}
		}
	}
	...
	if !wiringWarned {
		...
		if shell == "" || !a.isGatewayURL(shell) {
			if root == nil || !existed {
				fmt.Fprintf(a.out, "warning: claude wiring is off; the subagent toggle has no effect until 'openrouter-switch claude on'.\n")
...
// --- route (family pins) ---------------------------------------------------
//
// `openrouter-switch claude route` manages the per-client model_routes overlay
...
func (a *claudeAdapter) routeReloadAndVerify(key, wantTarget string) {
	...
	case pidfileAlive:
		...
		if waitRouteApplied(adminAddr, a.clientName, key, wantTarget, routeApplyTimeout) {
			...
			fmt.Fprintf(a.out, "note: SIGHUP sent (pid %d) but the router did not confirm the model route within %s; check 'openrouter-switch status'\n", pid, routeApplyTimeout)
		}
	default:
		fmt.Fprintf(a.out, "router not running; %s updated and applies at next start\n", a.configPath)
	}
...
func (a *claudeAdapter) routeWarnings() {
	...
	if err == nil && existed {
		if env, envErr := settingsEnv(root); envErr == nil {
			if base, ok := envString(env, claudeManagedEnvKey); ok && !a.isGatewayURL(base) {
				fmt.Fprintf(a.out, "warning: %s does not point at the gateway; family pins have no effect until 'openrouter-switch claude on'.\n", claudeManagedEnvKey)
				wiringWarned = true
			}
		}
	}
	if !wiringWarned {
		...
		if shell == "" || !a.isGatewayURL(shell) {
			if root == nil || !existed {
				fmt.Fprintf(a.out, "warning: claude wiring is off; family pins have no effect until 'openrouter-switch claude on'.\n")
			} else if env, envErr := settingsEnv(root); envErr == nil {
				if base, ok := envString(env, claudeManagedEnvKey); !ok || !a.isGatewayURL(base) {
					fmt.Fprintf(a.out, "warning: claude wiring is off; family pins have no effect until 'openrouter-switch claude on'.\n")
				}
