...
func TestHiddenHealthzHasDetailedHelp(t *testing.T) {
	...
	if !strings.HasPrefix(detail, "Usage: openrouter-switch healthz") ||
		!strings.Contains(detail, "OPENROUTER_SWITCH_GATEWAY_PORT") {
...
func TestDetailedHelpRetainsEnvironmentGuidance(t *testing.T) {
	tests := []struct {
		...
		{"up", []string{"OPENROUTER_SWITCH_LAUNCHD=off", "OPENROUTER_SWITCH_MENUBAR=off", "OPENROUTER_SWITCH_CONFIG_PATH"}},
	}
...
func TestDetailedHelpDoesNotClaimTelemetryDirOverride(t *testing.T) {
	for _, command := range []string{"gateway", "spend"} {
		t.Run(command, func(t *testing.T) {
			...
			if strings.Contains(out.String(), "OPENROUTER_SWITCH_TELEMETRY_DIR") {
				t.Fatalf("%s help claims unsupported OPENROUTER_SWITCH_TELEMETRY_DIR override:\n%s",
					command, out.String())
			}
		})
	}
...
func TestDispatchInterceptsLongAndShortCommandHelp(t *testing.T) {
	for _, helpFlag := range []string{"--help", "-h"} {
		t.Run(helpFlag, func(t *testing.T) {
			...
			if !strings.HasPrefix(out, "Usage: openrouter-switch up") {
...
func TestHelpIsInterceptedBeforeMutatingGatewayStop(t *testing.T) {
	...
	if !strings.HasPrefix(out, "Usage: openrouter-switch gateway") {
		t.Fatalf("help output = %q", out)
	}
