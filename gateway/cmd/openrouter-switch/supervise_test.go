...
func TestMain(m *testing.M) {
	...
	os.Setenv("OPENROUTER_SWITCH_MENUBAR", "off")
	code := m.Run()
	os.RemoveAll(dir)
	os.Exit(code)
}
...
func TestSuperviseState(t *testing.T) {
	...
	t.Run("OPENROUTER_SWITCH_LAUNCHD=off disables detection", func(t *testing.T) {
		f := useFake(t)
		f.respond("print", "ok", nil)
		installFakePlist(t, launchd.RouterLabel)
		...
		if len(f.calls) != 0 {
			t.Fatalf("launchctl consulted under OPENROUTER_SWITCH_LAUNCHD=off: %v", f.calls)
		}
	})
}

// TestInstallPlan pins the --install domain classification: foreign
// openrouter-switch labels (brew's etc.) refuse; our own labels are returned
...
func TestCmdUpInstallRefusesOnConflict(t *testing.T) {
	...
	if !strings.Contains(errOut, "homebrew.mxcl.openrouter-switch") {
...
func TestCmdUpInstallDisabled(t *testing.T) {
	...
	if rc != 1 || !strings.Contains(errOut, "OPENROUTER_SWITCH_LAUNCHD=off") {
...
func TestInstallAgents(t *testing.T) {
	...
	errOut := captureStderr(t, func() {
		if err := installAgents(lc, "/Users/x/.local/bin/openrouter-switch"); err != nil {
	...
	for i, label := range []string{launchd.RouterLabel, launchd.DoorLabel} {
		...
		b, _ := os.ReadFile(pp)
		content := string(b)
		wants := []string{label, "/Users/x/.local/bin/openrouter-switch", "OPENROUTER_SWITCH_CONFIG_PATH", "/tmp/gw.yaml",
...
func TestInstallAgentsNoDoorSection(t *testing.T) {
	...
	captureStderr(t, func() {
		if err := installAgents(lc, "/x/openrouter-switch"); err != nil {
			t.Errorf("installAgents: %v", err)
		}
	})
...
// TestWaitLabelGoneDisabled: OPENROUTER_SWITCH_LAUNCHD=off must short-circuit
...
func TestCmdRestartSupervisedRebootstrapsOnce(t *testing.T) {
	...
	pidDir := t.TempDir()
	t.Setenv("OPENROUTER_SWITCH_CONFIG_PATH", cfg)
	t.Setenv("OPENROUTER_SWITCH_ADMIN_ADDR", adminAddr)
	t.Setenv("OPENROUTER_SWITCH_GATEWAY_PIDFILE", filepath.Join(pidDir, "gw.pid"))
	t.Setenv("OPENROUTER_SWITCH_DOOR_PIDFILE", filepath.Join(pidDir, "door.pid"))
