...
func resetFixture(t *testing.T, contents string) string {
	...
	t.Setenv("OPENROUTER_SWITCH_GATEWAY_PIDFILE", filepath.Join(dir, "gateway.pid"))
	return path
}
...
func TestCmdConfigResetUsesStickyActivePath(t *testing.T) {
	...
	t.Setenv("OPENROUTER_SWITCH_CONFIG_PATH", "")
...
func TestCmdConfigResetBuildsIsolatedPreviewConfig(t *testing.T) {
	...
	t.Setenv("OPENROUTER_SWITCH_CONFIG_PATH", path)
	t.Setenv("OPENROUTER_SWITCH_GATEWAY_PIDFILE", filepath.Join(root, "gateway.pid"))
