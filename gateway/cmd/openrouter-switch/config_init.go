// config_init.go implements `openrouter-switch config init [--force]`, the
// commands-only config generator used by `openrouter-switch setup`. It writes
...
func cmdConfigInit(args []string) int {
	...
	for _, a := range args {
		...
		case "--force":
			force = true
		default:
			fmt.Fprintf(os.Stderr, "unknown flag: %s (usage: openrouter-switch config init [--force])\n", a)
			return 2
		}
	}
	path := envDefault("OPENROUTER_SWITCH_CONFIG_PATH", config.DefaultPath())
