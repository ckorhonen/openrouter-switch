...
import (
	...
	"github.com/ckorhonen/openrouter-switch/gateway/internal/doorcli"
...
func dispatch(args []string) int {
	...
	case "-h", "--help", "help":
		...
		return 0
	case "-v", "--version":
		fmt.Printf("%s %s\n", executableName(), version.Version)
		return 0
	}
...
var commandHelpEntries = []commandHelp{
	{"up", "Start the door and router, adopting the current binary", `Usage: openrouter-switch up [--install | --uninstall]
...
Environment:
  OPENROUTER_SWITCH_CONFIG_PATH     Explicit gateway.yaml path
  OPENROUTER_SWITCH_LAUNCHD=off     Disable launchd supervision and install/uninstall support
  OPENROUTER_SWITCH_MENUBAR=off     Skip the automatic macOS menubar app step
  OPENROUTER_SWITCH_DOOR_PIDFILE    Door pidfile path
  OPENROUTER_SWITCH_DOOR_LOG        Door log path
`},
	{"down", "Stop the door, router, and optional macOS menubar app", `Usage: openrouter-switch down
...
OPENROUTER_SWITCH_LAUNCHD=off disables launchd interaction. OPENROUTER_SWITCH_MENUBAR=off disables the
menubar shutdown step.
`},
	{"uninstall", "Remove the current OpenRouter Switch installation safely", `Usage: openrouter-switch uninstall [--dry-run] [--purge --yes]
...
app bundle in place.
`},
	{"status", "Show aggregate component, auth, and client routing health", `Usage: openrouter-switch status [--verbose|-v]
...
`},
	{"restart", "Stop and start the system using the same config", `Usage: openrouter-switch restart
...
rejected. Machine callers may place these options before or after the verb:
  ...
  --if-config-hash HASH
`},
	{"mutation", "Reconcile an interrupted routing policy mutation", `Usage: openrouter-switch mutation reconcile <operation-id> [--json]

Finish or safely roll back an interrupted routing policy mutation.
`},
	{"menubar", "Install or refresh and open the macOS menubar app", `Usage: openrouter-switch menubar [--which]
...
then open it. The installed location is Spotlight-indexable.
  ...
  --which  Print the openrouter-switch binary the menubar app will use
`},
	{"door", "Run the foreground front door", ""},
	{"gateway", "Manage the local router process directly", `Usage: openrouter-switch gateway <start|stop|restart|status> [--port INT] [--foreground]
...
Start and restart poll health before returning. When OPENROUTER_SWITCH_CONFIG_PATH is unset,
they reuse the config path recorded by the last gateway run.

Environment:
  OPENROUTER_SWITCH_CONFIG_PATH       Explicit gateway.yaml path
  OPENROUTER_SWITCH_GATEWAY_PORT      Router admin port, default 45273
...
`},
  ...
  openrouter-switch config init [--force]
  openrouter-switch config reset --yes [--preview-root PATH --router-addr HOST:PORT --door-addr HOST:PORT]
...
`},
	{"spend", "Summarize spend from local telemetry segments", `Usage: openrouter-switch spend
...
off restores the pre-on overlay exactly, deletes one created by openrouter-switch, or
...
const healthzHelp = `Usage: openrouter-switch healthz

Request the local gateway health endpoint and print its response.
OPENROUTER_SWITCH_GATEWAY_PORT selects the local port, default 45273.
`
...
func usage() {
	fmt.Fprintln(os.Stderr, "openrouter-switch - local rerouting gateway for AI coding harnesses")
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Usage: openrouter-switch <command> [options]")
	fmt.Fprintln(os.Stderr)
	...
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, "Run 'openrouter-switch <command> --help' for detailed help.")
}
...
func doorLogPath() string {
	return envDefault("OPENROUTER_SWITCH_DOOR_LOG", homeJoin(".config", "openrouter-switch", "logs", "door.log"))
}
...
func gatewayPortFlag(args []string) (int, bool, error) {
	port := envDefault("OPENROUTER_SWITCH_GATEWAY_PORT", strconv.Itoa(gateway.DefaultPort))
	foreground := false
...
func cmdGateway(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: openrouter-switch gateway [start|stop|restart|status] [--port INT] [--foreground]")
		return 2
	}
	sub := args[0]
	rest := args[1:]
...
func stickyConfigPath(explicitEnv, statePath string) (path, notice string) {
	...
	f.Close()
	return saved, fmt.Sprintf("reusing config path %s recorded by the last gateway run (OPENROUTER_SWITCH_CONFIG_PATH not set; export it to override)", saved)
}
...
func cmdGatewayStart(args []string) int {
	...
	if p, notice := stickyConfigPath(os.Getenv("OPENROUTER_SWITCH_CONFIG_PATH"), pidfile.ConfigStatePath(pidfile.Path())); p != "" || notice != "" {
		if notice != "" {
		...
		if p != "" {
			...
			os.Setenv("OPENROUTER_SWITCH_CONFIG_PATH", p)
		}
	}
