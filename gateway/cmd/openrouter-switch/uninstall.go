...
func parseUninstallOptions(args []string) (uninstallOptions, error) {
	...
	for _, arg := range args {
		...
		case "--yes":
			opts.yes = true
		default:
			return uninstallOptions{}, fmt.Errorf("unknown flag for uninstall: %s (usage: openrouter-switch uninstall [--dry-run] [--purge --yes])", arg)
		}
	}
...
func runUninstall(opts uninstallOptions, steps []uninstallStep, out io.Writer) int {
	...
	if opts.purge {
		fmt.Fprintln(out, "OpenRouter Switch uninstalled and current product data purged")
	} else {
		fmt.Fprintln(out, "OpenRouter Switch uninstalled; config, secrets, telemetry, logs, and backups were retained")
	}
	return 0
}
...
func defaultUninstallSteps() []uninstallStep {
	...
	return []uninstallStep{
		...
		{
			description: "restore or remove the Codex overlay only when OpenRouter Switch ownership can be proven",
			run:         uninstallCodex,
		},
		{
			description: "boot out and remove the current OpenRouter Switch launchd agents",
			run:         uninstallLaunchd,
		},
		{
			description: "stop the current OpenRouter Switch door and router",
			run:         uninstallProcesses,
		},
		{
			description: "quit the current OpenRouter Switch menubar app",
			run:         uninstallQuitMenubar,
		},
		{
			description: "remove current OpenRouter Switch runtime residue",
			run: func() error {
				return removeRuntimeResidue(root)
			},
		},
		{
			description: "verify the current OpenRouter Switch menubar app can be safely unregistered and removed, otherwise require manual removal",
...
func removeRuntimeResidue(root string) error {
	if err := validateOpenRouterSwitchDataRoot(root); err != nil {
		return err
	}
	var errs []error
...
func purgeOpenRouterSwitchDataRoot(root string) error {
	if err := validateOpenRouterSwitchDataRoot(root); err != nil {
		return err
	}
...
func validateOpenRouterSwitchDataRoot(root string) error {
	want := filepath.Clean(homeJoin(".config", "openrouter-switch"))
