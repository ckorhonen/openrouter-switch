...
//
//	-ldflags "-X github.com/ckorhonen/openrouter-switch/gateway/internal/version.Version=<git describe>"
//
// and stays "dev" for a plain `go build`. It is surfaced through
// `openrouter-switch --version`, the admin /healthz and /v1/admin/status
// payloads, and the door's /doorz payload, so `openrouter-switch status` can
