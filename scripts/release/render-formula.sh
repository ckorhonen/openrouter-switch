...
Usage: scripts/release/render-formula.sh \
  ...
  --output <Formula/openrouter-switch.rb path> \
...
artifact_name="openrouter-switch_${version}_darwin_universal.zip"
artifact_url="https://github.com/ckorhonen/openrouter-switch/releases/download/${tag}/${artifact_name}"
output_dir="$(dirname "$output")"
patch_output_dir="$(dirname "$patch_output")"
mkdir -p "$output_dir"
mkdir -p "$patch_output_dir"
tmp="$(mktemp "$output_dir/.openrouter-switch-formula.XXXXXX")"
patch_tmp="$(mktemp "$patch_output_dir/.openrouter-switch-formula-patch.XXXXXX")"
...
# This file is generated by OpenRouter Switch release automation. DO NOT EDIT.
class OpenrouterSwitch < Formula
  desc "Local gateway routing AI coding harnesses through OpenRouter"
  homepage "https://github.com/ckorhonen/openrouter-switch"
  url "${artifact_url}"
  sha256 "${sha256}"
  ...
  test do
    ...
    assert_path_exists pkgshare/"OpenRouter Switch.app.zip"
...
{
    printf 'diff --git a/Formula/openrouter-switch.rb b/Formula/openrouter-switch.rb\n'
    printf 'new file mode 100644\n'
    printf '%s\n' '--- /dev/null'
    printf '%s\n' '+++ b/Formula/openrouter-switch.rb'
    printf '@@ -0,0 +1,%s @@\n' "$formula_lines"
    sed 's/^/+/' "$tmp"
} >"$patch_tmp"
