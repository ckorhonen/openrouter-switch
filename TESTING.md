...
| `internal/*` | Analytics aggregation and indexing, config editing, Keychain/environment credential isolation, door circuit breaker, pidfiles, sanitization, telemetry, protocol translation, and usage accounting |
...
4. Verify the scratch ports are free. Defaults are 28081-28083, 28182-28183,
   ...
   worktrees.
5. Smoke: masked `auth status` and account catalog access using the configured
   OpenRouter credential. Skipped by `--offline` or when no local key exists.
...
front door returns 200 from OpenRouter and that telemetry records the request.

```sh
cd tests/fresh-install
OPENROUTER_API_KEY=... ./run.sh # keyed mode: real routed request
./run.sh --no-key               # keyless: asserts 503 needs-login, then a
                                # SIGHUP route flip to a local stub
```

The API key enters only via `docker run -e`, never an image layer. Not
simulated: macOS Keychain, the menubar app, Gatekeeper/notarization,
...
| `OPENROUTER_SWITCH_AUTH_NO_KEYRING=1` | disable Keychain access in hermetic tests; use only the inherited environment |
